...
import '../data/domain/repositories/ihome_repository.dart';
...
class HomeRepository implements IHomeRepository {
  ...
  @override
  Future<Responses> createRepost({required String originalThoughtId}) async {
    try {
      // Ensure original thought exists
      final original = await _firebaseService.readDocument(
        collectionPath: FirebaseFirestoreCollectionKeys.thoughts,
      ...
      if (original == null) {
        return Responses(
          success: false,
          message: 'Original thought not found.',
        );
      }

      // If original is a repost, flatten to its original
      final flattenedOriginalId = (original['type'] == 'repost' &&
              (original['originalThoughtId'] is String) &&
              (original['originalThoughtId'] as String).isNotEmpty)
          ? original['originalThoughtId'] as String
          : originalThoughtId;

      Map<String, dynamic>? flattenedOriginal = original;
      if (flattenedOriginalId != originalThoughtId) {
        flattenedOriginal = await _firebaseService.readDocument(
          collectionPath: FirebaseFirestoreCollectionKeys.thoughts,
          documentId: flattenedOriginalId,
        );
      }

      if (flattenedOriginal == null) {
        return Responses(
            success: false, message: 'Original thought not found.');
      }

      // Current user
      final user = _firebaseService.auth.currentUser;
      if (user == null) {
        return Responses(success: false, message: 'Not authenticated.');
      }

      // Build repost thought
      final newId = _firebaseService.firestore
          .collection(FirebaseFirestoreCollectionKeys.thoughts)
          .doc()
          .id;
      // Load current user's profile to embed repost author metadata
      final currentUserDoc = await _firebaseService.readDocument(
        collectionPath: FirebaseFirestoreCollectionKeys.users,
        documentId: user.uid,
      );
      AuthorMetadata? repostAuthorMetadata;
      if (currentUserDoc != null) {
        try {
          final userModel = UserModel.fromJson(currentUserDoc);
          repostAuthorMetadata = AuthorMetadata.fromUserModel(userModel);
        } catch (_) {}
      }

      // // Parse community metadata from original if present
      // CommunityMetadata? communityMetadata;
      // final originalCommunity = flattenedOriginal['communityMetadata'];
      // if (originalCommunity is Map<String, dynamic>) {
      //   communityMetadata = CommunityMetadata.fromJson(originalCommunity);
      // }

      final repost = ThoughtModel(
        id: newId,
        userId: user.uid,
        content: '',
        type: 'repost',
        ...
        originalThoughtId: flattenedOriginalId,
        originalUserId: flattenedOriginal['userId'] as String?,
        repostedAt: DateTime.now().toIso8601String(),
      );

      await _firebaseService.createDocument(
        collectionPath: FirebaseFirestoreCollectionKeys.thoughts,
        documentId: repost.id,
        data: repost.toJson(),
      );

      return Responses(success: true, message: 'Reposted');
    } catch (e) {
      return Responses(success: false, message: 'Failed to repost: $e');
    }
  }
  ...
  Future<Responses> sendVoiceThought({
    ...
    try {
      ...
      if (!file.existsSync()) {
        return Responses(success: false, message: 'Audio file does not exist.');
      }
      ...
      if (snapshot.state != TaskState.success) {
        return Responses(success: false, message: 'Failed to upload audio.');
      }
      ...
      // Persist thought with audio URL
      final voiceThought = ThoughtModel(
        id: thought.id,
        ...
        connectionOnly: thought.connectionOnly,
        authorMetadata: thought.authorMetadata,
        communityMetadata: thought.communityMetadata,
      );

      await _firebaseService.createDocument(
        collectionPath: FirebaseFirestoreCollectionKeys.thoughts,
        documentId: voiceThought.id,
        data: voiceThought.toJson(),
      );

      return Responses(success: true, message: 'Voice thought created.');
    } catch (e) {
      return Responses(
        success: false,
        message: 'Failed to create voice thought: ${e.toString()}',
      );
    }
  }
  ...
  Future<Responses> checkMelt({required String user2Id}) async {
    try {
      ...
      // Query to check if user2 has sent a melt request to user1 (unused but kept for potential future logic)
      await _firebaseService.firestore
          .collection(FirebaseFirestoreCollectionKeys.meltRequests)
